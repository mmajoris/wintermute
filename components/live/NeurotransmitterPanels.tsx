...
function SynapticRadial() {
  ...
  const vertices = NT_NAMES.map((name, i) => {
    ...
    return {
      ...
      labelX: cx + Math.cos(angle) * (maxR + 3),
      labelY: cy + Math.sin(angle) * (maxR + 3),
    };
  });
  ...
  return (
    <svg viewBox="-14 -14 228 228" className="w-full">
      <defs>
        {NT_NAMES.map((name, i) => {
          const color = NEUROTRANSMITTER_PATHWAYS[name]?.color || "#ff8833";
        ...
        return (
          <g key={`lb-${i}`}>
            <text x={v.labelX} y={v.labelY + dy}
              ...
              fontSize="10" fontWeight="700" fontFamily="system-ui, sans-serif" opacity="0.9"
            >
              {meta?.abbr || v.name}
            </text>
            <text x={v.labelX} y={v.labelY + dy + 10}
              textAnchor={anchor} fill="rgba(255,255,255,0.25)"
...
export function RadialPanel() {
  return (
    <BracketFrame variant="notched" className="px-5 pt-2 pb-3 shrink-0">
      <HudSectionTitle>Neurotransmitter Radial</HudSectionTitle>
      <div className="-mb-1">
        <SynapticRadial />
      </div>
    </BracketFrame>
...
export function TracesPanel() {
  return (
    <BracketFrame variant="detail-3" className="p-3 shrink-0">
      <HudSectionTitle>Signal Traces</HudSectionTitle>
      <div className="mt-1.5 space-y-1.5">
        {NT_NAMES.map((name) => (
          <NeuralTrace key={name} name={name} width={200} height={22} />
        ))}
      </div>
    </BracketFrame>
  );
